...
async fn launch_browser() -> Result<(Browser, tokio::task::JoinHandle<()>), Box<dyn std::error::Error>> {
...
async fn screenshot_page(page: &chromiumoxide::Page, name: &str) -> Result<(), Box<dyn std::error::Error>> {
...
async fn test_split_profiles_api() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // List all profiles
    let resp = client.get(format!("{}/api/split-profiles", base)).send().await?;
    assert!(resp.status().is_success(), "Split profiles list should succeed");
    let body: Value = resp.json().await?;
    let profiles = body.get("profiles").and_then(|v| v.as_array()).unwrap();
    assert!(profiles.len() >= 6, "Should have at least 6 built-in profiles, got: {}", profiles.len());
    ...
    for expected in &["openai", "grok", "perplexity", "claude", "gemini", "local"] {
        assert!(names.iter().any(|n| n == expected), "Profile '{}' should exist, got: {:?}", expected, names);
    }
    eprintln!("  Profiles: {:?}", names);

    // Get individual profile
    let resp = client.get(format!("{}/api/split-profiles/openai", base)).send().await?;
...
async fn test_split_with_profile() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let resp = client
        ...
        .send()
        .await?;
    assert!(resp.status().is_success(), "Split with profile should succeed");
    let body: Value = resp.json().await?;
    let chunks = body.get("chunks").and_then(|v| v.as_u64()).unwrap_or(0);
    assert!(chunks >= 2, "Should split into 2+ chunks with local profile (6KB), got: {}", chunks);
    ...
    eprintln!("  Local profile: {} chunks, {}B chunk_size, {}B overlap",
    ...
    if contents.len() >= 2 {
        ...
        assert!(chunk2.starts_with(overlap_text),
            "Chunk 2 should start with overlap from chunk 1");
        eprintln!("  Overlap verified: chunk2 starts with last {} chars of chunk1", overlap_text.len());
    }
    ...
    assert!(body.get("available").is_some(), "Should list available profiles");
...
async fn test_splitter_profile_selector() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let active: String = eval(&page,
        "document.querySelector('.profile-card.active')?.dataset.name || 'none'"
    ).await?.as_str().unwrap_or_default().to_string();
    assert_eq!(active, "openai", "OpenAI should be selected by default, got: {}", active);
    ...
    let chunk_val: String = eval(&page, "document.getElementById('chunkSize').value")
        .await?.as_str().unwrap_or_default().to_string();
    assert_eq!(chunk_val, "100000", "Chunk size should be 100000 for OpenAI, got: {}", chunk_val);

    // Check that overlap was set
    let overlap_val: String = eval(&page, "document.getElementById('overlap').value")
        .await?.as_str().unwrap_or_default().to_string();
    assert_eq!(overlap_val, "2000", "Overlap should be 2000 for OpenAI, got: {}", overlap_val);
...
async fn create_test_paste(client: &reqwest::Client, base: &str, title: &str, content: &str) -> Result<String, Box<dyn std::error::Error>> {
    let resp = client
        .post(format!("{}/paste", base))
        .json(&serde_json::json!({ "title": title, "content": content }))
        .send()
        .await?;
    assert!(resp.status().is_success(), "Paste creation failed: {}", resp.status());
    let body: Value = resp.json().await?;
    Ok(body.get("id").and_then(|v| v.as_str()).unwrap_or("unknown").to_string())
}
...
async fn test_splitter_page_loads() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(title.contains("Splitter"), "Title should contain 'Splitter', got: {}", title);
    ...
    assert!(btn_text.as_ref().map(|s| s.contains("Split")).unwrap_or(false),
        "First button should be Split, got: {:?}", btn_text);
...
async fn test_split_api() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let resp = client
        ...
        .send()
        .await?;
    assert!(resp.status().is_success(), "Split API should succeed: {}", resp.status());
    let body: Value = resp.json().await?;
    let chunks = body.get("chunks").and_then(|v| v.as_u64()).unwrap_or(0);
    assert!(chunks > 1, "Should split into multiple chunks with chunk_size=50, got: {}", chunks);
    ...
    let resp = client
        ...
        .send()
        .await?;
    assert!(resp.status().is_success(), "Split-upload API should succeed: {}", resp.status());
    let body: Value = resp.json().await?;
...
async fn test_paste_create_and_view() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let has_form = page.find_element("form").await.is_ok()
        || page.find_element("textarea").await.is_ok();
    assert!(has_form, "Home page should have a form or textarea for paste creation");

    screenshot_page(&page, "home-page").await.ok();

    // Create a paste via the API
    let client = reqwest::Client::new();
    let paste_id = create_test_paste(&client, &base, "Headless Test Paste",
        "This is a test paste created by the headless browser test suite.").await?;

    // Navigate to the paste view page
    let page2 = browser.new_page(format!("{}/paste/{}", base, paste_id)).await?;
    ...
    assert!(title.contains("Headless Test Paste") || title.contains("kant-pastebin"),
        "Title should contain paste title or kant-pastebin, got: {}", title);
...
async fn test_reply_button() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create a paste
    let client = reqwest::Client::new();
    let paste_id = create_test_paste(&client, &base, "Reply Test Paste",
        "Testing the reply button functionality.").await?;

    // Navigate to the paste page
    let page = browser.new_page(format!("{}/paste/{}", base, paste_id)).await?;
    page.wait_for_navigation_response().await?;
    wait_ms(1000).await;
    ...
    if let Ok(link) = reply_link {
        ...
        assert!(text_str.contains("Reply"), "Reply link text should contain 'Reply', got: {}", text_str);

        // Check the href contains reply_to
        let href = link.attribute("href").await?;
        assert!(href.as_ref().map(|h| h.contains("reply_to=")).unwrap_or(false),
            "Reply link href should contain reply_to=, got: {:?}", href);
...
async fn test_copy_button() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create a paste
    let client = reqwest::Client::new();
    let paste_id = create_test_paste(&client, &base, "Copy Test Paste",
        "Testing the copy button.").await?;
...
async fn test_share_button() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create a paste
    let client = reqwest::Client::new();
    let paste_id = create_test_paste(&client, &base, "Share Test Paste",
        "Testing the share button.").await?;
    ...
    // Check the Share button exists (uses navigator.share)
    let share_btn: bool = eval(&page,
        "Array.from(document.querySelectorAll('button.reply-btn')).some(b => b.textContent.includes('Share'))"
    ).await?.as_bool().unwrap_or(false);

    assert!(share_btn, "Share button should exist on paste page");

    // Mock navigator.share to capture the call
    eval(&page, r#"
    ...
    eval(&page, "Array.from(document.querySelectorAll('button.reply-btn')).find(b => b.textContent.includes('Share')).click()").await?;
    wait_ms(500).await;

    // Check navigator.share was called
    let share_call: Value = eval(&page, "window.__shareCall").await?;
    assert!(!share_call.is_null(), "navigator.share should have been called");
    let share_url = share_call.get("url").and_then(|v| v.as_str()).unwrap_or_default();
    assert!(share_url.contains(&paste_id), "Share URL should contain paste ID, got: {}", share_url);
...
async fn test_split_button_on_paste_page() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create a paste
    let client = reqwest::Client::new();
    let paste_id = create_test_paste(&client, &base, "Split Test Paste",
        "Testing the split button from paste page.").await?;
    ...
    assert!(!stored_str.is_empty(), "localStorage should have splitter-text after clicking Split");
...
async fn test_all_buttons_present() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create a paste
    let client = reqwest::Client::new();
    let paste_id = create_test_paste(&client, &base, "Button Inventory Test",
        "Checking all buttons exist.").await?;
    ...
    for label in &expected {
        assert!(buttons.contains(label), "Button '{}' should be present, got: {}", label, buttons);
    }
